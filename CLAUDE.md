...
| `ObColor` | `#C9A227` | Dark yellow — OB (out of bounds) button and counts |
...
- **Player cards:** left `Column` (`weight(1f)`) stacks player name (28sp, `FontWeight.Bold`, `ContentWhite`) above round vs-par score (`titleSmall`, `ContentWhite`, hidden as `"•••"` when scores hidden); on the right, the **OB counter** for the current hole (`ObColor` dark yellow, `ExtraBold` 15sp; tap cycles `"- OB"` → `"1 OB"` → `"2 OB"` → `"3+ OB"` → back to `"- OB"`, long-press steps one back, via `combinedClickable`; writes through `RoundViewModel.setOb`, stored count caps at 3) sits **left of** the plain `IconButton` −/+ score controls; cards are full screen width (no horizontal padding on the `LazyColumn`). The round OB total is intentionally not shown on the card — it appears in Review/Detail only
...
- **OB display:** each player's round OB total appears at the **bottom-left** of their scorecard block (`"N OB"`, `bodyMedium` Bold, `ObColor`, only when > 0) — same placement in `RoundReviewScreen`'s player cards and `FullScorecardSheet`.
